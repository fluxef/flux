...
class Uri implements UriInterface
{
    ...
    protected function init(string $uri): void
    {
        ...
        if ($comp !== false) {
            ...
            if (!empty($comp['scheme']))
                $this->scheme = strtolower($comp['scheme']);

            if (!empty($comp['host']))
                $this->host = $comp['host'];

            if ((isset($comp['port'])) && ($comp['port'] > 0))
                $this->port = $comp['port'];
            ...
            if (!empty($comp['query']))
                $this->query = $comp['query'];

            if (!empty($comp['fragment']))
                $this->fragment = $comp['fragment'];
        }
    ...
    public function getPort(): ?int
    {
        ...
        if (self::PORTS[$this->scheme] == $this->port)
            return null;
    ...
    public function withPort(?int $port): static
    {
        ...
        $obj = clone($this);
        $obj->port = (int)$port;    // only for code sniffer
        return $obj;
    }
    ...
    protected function isRootless(string $path): bool
    {
        if (empty($path))
            return true;
    ...
    public function __toString(): string
    {
        ...
        if (!empty($auth) && $this->isRootless($this->path))
            $uri .= '/' . $this->path;
        elseif (empty($auth) && !$this->isRootless($this->path))
            $uri .= '/' . ltrim($this->path, '/');
        else
            $uri .= $this->path;
